...
(function() {
  var baseLayerButton, bingAerial, bingHybrid, bingKey, bingRoad, defaultProjection, geoLocateControl, geoLocateVector, getMLLXYZ, getTileURL, htmlLunch, layerPanel, loadMapPosition, lolnasLayer, maastoButton, map, mapPosition, mapQuest, mmlIlmakuvat, mmlMaastoKartat, onPopupClose, onPopupFeatureSelect, onPopupFeatureUnselect, osmCycle, osmStandard, osmTransport, oviSatellite, oviStreets, oviTransit, restaurantPopupHtml, storeMapPosition, styleMap, trafiMerikartta, transformLonLat;
  ...
  baseLayerButton = new OpenLayers.Control({
    ...
    eventListeners: {
      activate: function() {
        if (mapQuest) {
          return map.setBaseLayer(mapQuest);
        }
      }
    }
  });
  ...
  maastoButton = new OpenLayers.Control({
    ...
    eventListeners: {
      activate: function() {
        if (mmlMaastoKartat) {
          return map.setBaseLayer(mmlMaastoKartat);
        }
      }
    }
  });
